...
export class Client extends AmeriaClient {
  ...
  async initPayment(params: IInitPaymentParams): Promise<IInitPaymentResponse & { url: string | null }> {
    ...
    const data: IInitPaymentResponse = await this.ameriaFetch('InitPayment', body);
  ...
  async confirmPayment(paymentId: string, amount: number): Promise<IConfirmPaymentResponse> {
  ...
  async refundPayment(paymentId: string, amount: number): Promise<IRefundPaymentResponse> {
  ...
  async activateBinding(cardHolderId: string, paymentType: keyof typeof PaymentTypes) {
  ...
  async deactivateBinding(cardHolderId: string, paymentType: keyof typeof PaymentTypes) {
    const num = PaymentTypes[paymentType];
    ok(num);
  ...
  async makeBindingPayment(params: IMakeBindingPaymentParams) {
    ...
    const data: IMakeBindingPaymentResponse = await this.ameriaFetch('MakeBindingPayment', body);
