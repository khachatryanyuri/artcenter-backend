...
export const RESPONSE_CODES: Record<string, IResponseCode> = {
  '0-1': {
  ...
  '0-2001': {
    description: 'Transaction declined since Client’s IP address is in the blacklist',
    message: 'Decline. IP blacklisted',
  },
  '0-20010': {
    description: 'Transaction declined since payment amount exceeded the limits set by the issuer bank',
  ...
  '0-2005': {
    description: 'We failed to check the issuer’s signature, i.e. PARes was readable but was not signed correctly',
  ...
  '0-2019': {
    description: 'Issuer’s PARes contains iReq, due to which payment was declined.',
  ...
  '01': {
    description: 'Order with the given number is already registered in the system',
  ...
  '01001': {
    description: 'At the moment of registering the transaction, i.e. when the card details are not entered yet',
  ...
  '0103': {
    description: 'No connection with the issuer bank, the merchant must call the issuer bank',
  ...
  '0109': {
    description: 'Merchant’s/*terminal ID is invalid (for completion and preauthorization with different IDs)',
  ...
  '0121': {
    description: 'Attempt to make a transaction exceeding the daily limit set by the issuer bank',
  ...
  '0151019': {
    description: 'Processing timeout, sent but no response received from the bank',
    message: 'Decline. Processing timeout',
  },
  '02001': {
    description: 'Fraudulent transaction ( according to processing or payment network)',
    message: 'Decline. Fraud',
  },
  '02003': {
    description: 'Merchant is not allowed to perform SSL (Not 3d-Secure/SecureCode) transactions',
  ...
  '08204': {
    description: 'Such order has already been registered (check by order number)',
  ...
  '0903': {
    description: 'Attempt to make a transaction in the amount exceeding the limit set by the issuer bank',
  ...
  '0909': {
    description: 'General system failure fixed by the payment network or the issuer bank',
    message: 'Decline. Call issuer',
  },
  '0910': {
  ...
  '0914': {
    description: 'Transaction is not found (while sending completion, reversal or refund request)',
    message: 'Decline. Orig trans not found',
  },
  '0999': {
    description: 'Transaction authorization did not start. Declined due to fraud or 3dsec error.',
